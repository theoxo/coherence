...
class mu_1_MessageType: public mu__byte
{
 public:
  ...
  mu_1_MessageType (const char *name, int os): mu__byte(3, 14, 4, name, os) {};
  mu_1_MessageType (void): mu__byte(3, 14, 4) {};
  mu_1_MessageType (int val): mu__byte(3, 14, 4, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
const char *mu_1_MessageType::values[] = {"ReadReq","WriteReq","WBReq","AckRead","AckWrite","AckWB","AckData","DataReqS","DataReqI","DataNoReqI","DataResp","Inv",NULL };
...
class mu_1__type_1: public mu__byte
{
 public:
  ...
  mu_1__type_1 (const char *name, int os): mu__byte(15, 19, 3, name, os) {};
  mu_1__type_1 (void): mu__byte(15, 19, 3) {};
  mu_1__type_1 (int val): mu__byte(15, 19, 3, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
class mu_1__type_3: public mu__byte
{
 public:
  ...
  mu_1__type_3 (const char *name, int os): mu__byte(20, 30, 4, name, os) {};
  mu_1__type_3 (void): mu__byte(20, 30, 4) {};
  mu_1__type_3 (int val): mu__byte(20, 30, 4, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
const char *mu_1__type_3::values[] = {"PM","PS","PI","PIMad","PIMa","PIMd","PSMa","PMIa","PSI","PMS","PIIa",NULL };
...
const int mu_AckRead = 6;
const int mu_AckWrite = 7;
const int mu_AckWB = 8;
const int mu_AckData = 9;
const int mu_DataReqS = 10;
const int mu_DataReqI = 11;
const int mu_DataNoReqI = 12;
const int mu_DataResp = 13;
const int mu_Inv = 14;
const int mu_HM = 15;
const int mu_HS = 16;
const int mu_HI = 17;
...
const int mu_PS = 21;
const int mu_PI = 22;
const int mu_PIMad = 23;
const int mu_PIMa = 24;
const int mu_PIMd = 25;
const int mu_PSMa = 26;
const int mu_PMIa = 27;
const int mu_PSI = 28;
const int mu_PMS = 29;
const int mu_PIIa = 30;
...
mu_Send ( mu_AckRead, mu_msg.mu_src, (int)mu_HomeType, mu_VC2, mu_msg.mu_aux, (int)mu_msg.mu_cnt );
break;
case mu_WriteReq:
mu_HomeNode.mu_state = mu_HM;
mu_HomeNode.mu_owner = mu_msg.mu_src;
mu_Send ( mu_AckWrite, mu_msg.mu_src, (int)mu_HomeType, mu_VC2, mu_msg.mu_aux, (int)mu_msg.mu_cnt );
break;
default:
...
switch ((int) mu_msg.mu_mtype) {
case mu_ReadReq:
mu_Send ( mu_DataReqS, mu_HomeNode.mu_owner, (int)mu_HomeType, mu_VC1, mu_msg.mu_src, (int)mu_msg.mu_cnt );
...
break;
case mu_WriteReq:
mu_Send ( mu_DataReqI, mu_HomeNode.mu_owner, (int)mu_HomeType, mu_VC1, mu_msg.mu_src, (int)mu_msg.mu_cnt );
mu_Send ( mu_AckWrite, mu_msg.mu_src, (int)mu_HomeType, mu_VC2, mu_msg.mu_aux, (int)mu_msg.mu_cnt );
break;
case mu_WBReq:
mu_Send ( mu_DataNoReqI, mu_HomeNode.mu_owner, (int)mu_HomeType, mu_VC1, mu_msg.mu_aux, (int)mu_msg.mu_cnt );
mu_HomeNode.mu_state = mu_HId;
break;
case mu_DataResp:
mu_Send ( mu_AckData, mu_msg.mu_src, (int)mu_HomeType, mu_VC2, mu_1_Node_undefined_var, mu_1__type_7_undefined_var );
break;
default:
mu_ErrorUnhandledMsg ( mu_msg, (int)mu_HomeType );
break;
...
mu_Send ( mu_Inv, (int)mu_proc, (int)mu_HomeType, mu_VC0, mu_1_Node_undefined_var, (int)mu_msg.mu_cnt );
};
};
mu_HomeNode.mu_state = mu_HM;
mu_HomeNode.mu_owner = mu_msg.mu_src;
...
case mu_DataResp:
mu_HomeNode.mu_state = mu_HS;
mu_Send ( mu_AckData, mu_msg.mu_src, (int)mu_HomeType, mu_VC2, mu_1_Node_undefined_var, (int)mu_msg.mu_cnt );
break;
default:
mu_ErrorUnhandledMsg ( mu_msg, (int)mu_HomeType );
break;
...
case mu_DataReqS:
mu_ps = mu_PMS;
mu_Send ( mu_DataResp, (int)mu_HomeType, (int)mu_p, mu_VC0, mu_1_Node_undefined_var, (int)mu_msg.mu_cnt );
mu_Send ( mu_DataResp, mu_msg.mu_aux, (int)mu_p, mu_VC0, mu_1_Node_undefined_var, (int)mu_msg.mu_cnt );
break;
case mu_DataReqI:
mu_ps = mu_PMIa;
mu_Send ( mu_DataResp, (int)mu_HomeType, (int)mu_p, mu_VC0, mu_1_Node_undefined_var, (int)mu_msg.mu_cnt );
...
bool mu__boolexpr13;
bool mu__boolexpr14;
  if ((mu_msg.mu_vc) == (mu_VC2)) mu__boolexpr14 = TRUE ;
  else {
bool mu__boolexpr15;
  if (!((mu_msg.mu_vc) == (mu_VC1))) mu__boolexpr15 = FALSE ;
  else {
/*** begin multisetcount 3 declaration ***/
  ...
  for (mu_m = 0; ; mu_m=mu_m+1)
	...
	{
	  if ( (mu_chan[mu_m].mu_vc) == (mu_VC2) ) mu__intexpr16++;
	}
      if (mu_m == 2-1) break;
    }
  }
...
bool mu__boolexpr17;
bool mu__boolexpr18;
  if (!((mu_msg.mu_vc) == (mu_VC0))) mu__boolexpr18 = FALSE ;
  else {
  ...
  for (mu_m = 0; ; mu_m=mu_m+1)
    {
      if (mu_chan.valid[(int)mu_m].value())
	{
	  if ( (mu_chan[mu_m].mu_vc) == (mu_VC2) ) mu__intexpr19++;
	}
      if (mu_m == 2-1) break;
    }
  }
/*** end multisetcount 4 declaration ***/
  mu__boolexpr18 = ((mu__intexpr19) == (0)) ; 
}
  ...
  for (mu_m = 0; ; mu_m=mu_m+1)
    {
      if (mu_chan.valid[(int)mu_m].value())
	{
	  if ( (mu_chan[mu_m].mu_vc) == (mu_VC1) ) mu__intexpr20++;
	}
      if (mu_m == 2-1) break;
    }
  }
/*** end multisetcount 5 declaration ***/
  mu__boolexpr17 = ((mu__intexpr20) == (0)) ; 
}
  mu__boolexpr13 = (mu__boolexpr17) ; 
}
...
bool mu__boolexpr21;
bool mu__boolexpr22;
  if ((mu_msg.mu_vc) == (mu_VC2)) mu__boolexpr22 = TRUE ;
  else {
bool mu__boolexpr23;
  if (!((mu_msg.mu_vc) == (mu_VC1))) mu__boolexpr23 = FALSE ;
  else {
/*** begin multisetcount 3 declaration ***/
  int mu__intexpr24 = 0;
  {
  ...
  for (mu_m = 0; ; mu_m=mu_m+1)
    {
      if (mu_chan.valid[(int)mu_m].value())
	{
	  if ( (mu_chan[mu_m].mu_vc) == (mu_VC2) ) mu__intexpr24++;
	}
      if (mu_m == 2-1) break;
    }
  }
...
bool mu__boolexpr25;
bool mu__boolexpr26;
  if (!((mu_msg.mu_vc) == (mu_VC0))) mu__boolexpr26 = FALSE ;
  else {
/*** begin multisetcount 4 declaration ***/
  int mu__intexpr27 = 0;
  {
  ...
  for (mu_m = 0; ; mu_m=mu_m+1)
    {
      if (mu_chan.valid[(int)mu_m].value())
	{
	  if ( (mu_chan[mu_m].mu_vc) == (mu_VC2) ) mu__intexpr27++;
	}
      if (mu_m == 2-1) break;
    }
  }
  ...
  for (mu_m = 0; ; mu_m=mu_m+1)
    {
      if (mu_chan.valid[(int)mu_m].value())
	{
	  if ( (mu_chan[mu_m].mu_vc) == (mu_VC1) ) mu__intexpr28++;
	}
      if (mu_m == 2-1) break;
    }
  }
/*** end multisetcount 5 declaration ***/
  mu__boolexpr25 = ((mu__intexpr28) == (0)) ; 
}
...
public:
  ...
  void Code(unsigned r)
  {
    ...
    r = r / 1;
  mu_1_ProcState& mu_p = mu_Procs[mu_n];
mu_Send ( mu_WBReq, (int)mu_HomeType, (int)mu_n, mu_VC2, mu_1_Node_undefined_var, mu_1__type_7_undefined_var );
  };
...
/********************
  Invariant records
 ********************/
int mu__invariant_29() // Invariant "modified w/ empty sharers list"
{
bool mu__boolexpr30;
  if (!((mu_HomeNode.mu_state) == (mu_HM))) mu__boolexpr30 = TRUE ;
  else {
...
bool mu__condition_32() // Condition for Rule "modified w/ empty sharers list"
{
...
{"modified w/ empty sharers list", &mu__condition_32, NULL, },
};
